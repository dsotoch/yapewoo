<?php
/**
 * Plugin Name: Pago Yape ViruTec
 * Description: El plugin " Pago Yape ViruTec" permite agregar YAPE como forma de pago a su tiendas WooCommerce. YAPE es un medio de pago peruano, por lo que el plugin es para Perú, YAPE permite pagos con su telefono Smartphone.
 * Version: 1.2
 * Author: Diego Soto - Viru Tec
...
define('PAGO_MOVILES_PERU_VERSION', '1.2');
...
define('PAGO_MOVILES_PERU_DB_TABLE', $wpdb->prefix . 'pago_yape_virutec');
define('YAPE_VIRUTEC', $wpdb->prefix . 'pago_yape');
    ...
    class PagoMovilesPeru_Gateway extends WC_Payment_Gateway
    {
        ...
        public function payment_fields()
        {
            ...
            echo sprintf( wp_kses( '<div class="form-row form-row-wide">
                ...
                <span class="text-bold">Nombres Completos de titular del Yape</span><br/>
                <input type="text" name="titular-yape" id="titular-yape" placeholder="NOMBRES COMPLETOS del titular de Yape" oninput="this.value = this.value.toUpperCase();" />
                </div>
        ...
        {
            if(empty( $_POST['titular-yape'] ) && $_POST['payment_method'] == 'pago_moviles_peru' ) {
                wc_add_notice( 'El campo del Titular del Yapeo no puede quedar vacio!', 'error' );
                return false;
            }
        ...
        public function process_payment( $order_id )
        {
            ...
            $pagador = isset($_POST['titular-yape']) ? $_POST['titular-yape'] : '';
            $pagador = strtoupper(str_replace(' ', '', $pagador));            
